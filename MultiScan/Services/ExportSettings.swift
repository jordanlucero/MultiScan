...
//  MultiScan
//
//  User preferences for text export separators, persisted in UserDefaults.
//
...
/// Observable export settings. Values are cached in stored properties and written through to UserDefaults on change (`didSet` rather than `@AppStorage`, so `@Observable` tracking works).
@MainActor
@Observable
